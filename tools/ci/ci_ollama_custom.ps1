<#
CI-safe replacement for `ollama-sandbox-writable-custom.wsb`.
Same as `ci_ollama_writable.ps1` but keeps a separate output path for custom artifacts.
#>
param(
    [string]$Version = 'v0.20.4',
    [string]$OutRoot = "$PSScriptRoot\bootstrap\ollama_custom",
    [switch]$RunInstaller
)
